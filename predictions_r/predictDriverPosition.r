...
################### Train Models ###################
model_qrf <- quantregForest(x = set, y = set$positionOrder, nthreads = 2)
model_lm <- lm(positionOrder ~ points + grid + fastestLapSpeed + statusId, data=set)
model_rf <- randomForest(positionOrder ~ points + grid + fastestLapSpeed + statusId, data=set, proximity=TRUE)
